  ...
  <body>	
        ...
        <div class="container p-1" >
			<div class="row">
				...
				<div class="col nav justify-content-end">
					<div class="login my-2" style="cursor : pointer ; border-bottom : 5px solid yellow" onclick="home()"> <i class="fas fa-home text-white ">&nbsp&nbspHome</i></div><i class="my-2 text-white">&nbsp&nbsp&nbsp&nbsp|&nbsp&nbsp&nbsp&nbsp</i>
					...
					<div class="login my-2" style="cursor : pointer " onclick="wasted()"> <i class="fas fa-times text-white ">&nbsp&nbspWasted</i></div><i class="my-2 text-white">&nbsp&nbsp&nbsp&nbsp|&nbsp&nbsp&nbsp&nbsp</i>
	...
	<script>
		...
		function showDetails(ID , userID , name , report , farmer , location , phone , image , flag ) {
			...
			document.getElementById('pNameId').value = name;
			document.getElementById('user').value = userID;
			document.getElementById('tags').value = name + "_" + ID;
			if(flag == 0){
  ...
  <div class="modal-dialog modal-dialog-aside" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
		...
		<button type="button" class="btn btn-primary" onclick="openMessageBox()"><i class="fas fa-comments"></i></button>
		<p id="report"></p>
		<p id="farmer" class="fas fa-user"></p> <br>
		<p id="location" class="fas fa-map-marker-alt"></p> <br>
		<p id="phone" class="fas fa-phone-alt"></p>
		
		<p><form method="POST" id="flag"  style="display : none;">
			<select name="flag" id="items" style="width : 100%;">
				<option value="s">--Select the product--</option>
				<option value="1"> Bad</option>	
				<option value="2">Good</option>	
			</select>
			<input type="text" class="form-control" style="display : none" id="Flagid" name = "id">
			<button type="submit" id="btn" class="btn btn-success my-2" name="sf">Set Flag</button>
		</form></p>
		<hr>
		<h6 class="text-secondary">Keels > Buy Product ></h6>
		<form  method="POST">
			
			<div class="mb-3">
				<input type="text" class="form-control" style="display : none" id="pNameId" name = "pName">
				<label for="exampleInputPassword1" class="form-label">Quantity</label>
				<input type="text" class="form-control" name = "pQty">
			</div>
			<button type="submit" class="btn btn-success" name="bn">Buy Now</button>
		</form>
      </div>
...
<div class="modal fade" id="message" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
		<form  method="POST">
			<input type="text" class="form-control" id="user" name="userID" aria-describedby="emailHelp" style="display : none">
			<input type="text" class="form-control" id="tags" name="sTags" aria-describedby="emailHelp" style="display : none">
			<div class="mb-3">
				<label for="exampleInputPassword1" class="form-label">Your Message</label>
				<textarea class="form-control"  name="Message" placeholder="your message here..." style="height: 100px"></textarea>
			</div>
			<button type="submit" class="btn btn-success" name="msg">Send Message</button>
		</form>
	...
	<?php
		...
		if(isset($_POST["msg"])){	
			...
			$Tags = $_POST['sTags'];
			$msg = $_POST['Message'];
			$sendersName = $_SESSION['Name'];
			$reply = "";
			$con = getConnection();
			$sql2 = "INSERT into message (ID , userID , sendersName , Message , Tags , reply) 
			value(NULL , '$userID' , '$sendersName', '$msg' , '$Tags' , '$reply') ";
			if($con->query($sql2)) {
